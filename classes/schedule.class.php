...
Class Schedule{
	...
	public static function remove_from_schedule($user_id, $lecture_id) {
		...
		$stmt = $conn->prepare("DELETE FROM Schedule WHERE id_user = ? AND id_lecture = ?");
		$stmt->bind_param('ii', $user_id, $lecture_id);

		if (!$stmt->execute()) {
			self::$error_message = 'Chyba pri odstraňovaní prednášky z rozvrhu.';
			$db->close();
			return false;
		};
		...
		self::$error_message = 'Prednáška bola úspešne odstránená z rozvrhu.';
		return true;
    }
	...
	public static function get_user_schedule($user_id, $time_start, $time_end) {
		...
		$stmt = $conn->prepare('SELECT l.id, l.name as name, l.time_from, l.time_to, r.name as room'
							  ...
							  .' AND l.time_from >= ? AND l.time_from < ? AND l.status = 1 ORDER BY l.time_from');
